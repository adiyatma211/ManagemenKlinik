...
@section('konten')
                            ...
                            <tbody>
                                @foreach ($ShowPatien as $key => $a)
                                    <tr>
                                        ...
                                        <td> {{ \Carbon\Carbon::parse($a->tgl_periksa)->format('d-m-Y') }}
                                        </td>
                                        <td>{{ empty($a->konfirmasi) || $a->konfirmasi == 0 ? 'Belum Konfirmasi' : ($a->konfirmasi == 1 ? 'Sudah Konfirmasi' : $a->konfirmasi) }}</td>
                                        <td>{{ $a->createdBy }}</td>
                                        
                                        <td>
                                            <div class="d-flex justify-content-between">
                                                <button class="btn btn-sm me-1 {{ $a->konfirmasi == 1 ? 'btn-secondary' : 'btn-warning' }}" 
                                                    onclick="confirmAttendance('{{ $a->no_rm }}')" 
                                                    {{ $a->konfirmasi == 1 ? 'disabled' : '' }}>
                                                Konfirmasi
                                            </button>
                                                <button class="btn btn-info btn-sm me-1" onclick="viewPatient('{{ $a->no_rm }}')">View</button>
                                                <button class="btn btn-primary btn-sm me-1" onclick="openPatientModal('edit', '{{ $a->no_rm }}')">Edit</button>
                                                <button class="btn btn-danger btn-sm" onclick="deletePasien('{{ $a->no_rm }}')">Delete</button>
                                            </div>
                                        </td>
 ...
 function viewPatient(no_rm) {
     ...
     function confirmAttendance(no_rm) {
    Swal.fire({
        title: 'Apakah Anda yakin ingin mengonfirmasi kehadiran?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Terjadi kesalahan saat mengonfirmasi kehadiran.'
                });
            });
        }
    });
